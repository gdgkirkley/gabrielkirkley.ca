...
the same videos in each list. Plus, we had a feature section which displayed
"more videos" to the user and it would be kinda boring if the same two videos
were always listed there. A true machine learning model for recommending videos
was outside the scope of this project (and totally unnecessary), so I needed to
be able to retrieve data from the database at random, but also allow for
pagination.
...
Originally, I had it set up to just use a single modulus (7) but it quickly
became apparent that this presented the same problem: the results were shuffled,
but they were always shuffled exactly the same! Getting a truly random number as
the modulus was key.
...
Now, the user got random videos - but they were duplicated again and again! In
order to prevent duplicates, the order fetched from the database needed to be
repeatable. So I knew that this was a job for some good old React state.

Now, on the frontend, I get a random number between 2 and 11 and store it in
state.
